...
export const metadata: Metadata = buildLandingMetadata("/blurry-photo")

export default function BlurryPhotoPage() {
  const landing = getLandingConfig("/blurry-photo")!
  ...
  return (
    <main className="min-h-screen flex flex-col bg-neutral-50">
      ...
      {landing.howItWorksSteps.length > 0 ? (
        <BlurryHowToSection title={landing.howItWorksTitle} steps={landing.howItWorksSteps} />
      ) : null}
      {landing.faqItems && landing.faqItems.length > 0 ? (
        <FaqSection title={landing.faqTitle} faqs={landing.faqItems} />
      ) : null}
      <NewsletterSection />
      <Footer />
      <PreviewModal />
...
function BlurryHowToSection({ title, steps }: { title: string; steps: StepItem[] }) {
  return (
